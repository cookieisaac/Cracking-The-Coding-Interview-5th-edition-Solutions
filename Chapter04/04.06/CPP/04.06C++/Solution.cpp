...
TreeNode* Tree::_findNodeHelper(int value, TreeNode* node)
{
	if (node == NULL)
		return NULL;
	...
	TreeNode* left = _findNodeHelper(value, node->getLeft());
	TreeNode* right = _findNodeHelper(value, node->getRight());
...
TreeNode* Tree::_leftMostChild(TreeNode* node)
{
	if (node == NULL)
	{
		return NULL;
...
TreeNode* Tree::inorderSuccessor(TreeNode* node)
{
	...
	else
	{
		if (node->getRight() != NULL)
		{
			return _leftMostChild(node->getRight());
		}
		else
		{
			TreeNode* temp = node->getParent();
			while (temp && node!=temp->getLeft())
			{
				node = temp;
				temp = node->getParent(); 
			}
