...
public:
	...
	void createListByDepth(std::vector<std::list<TreeNode*>>&);
	void printLayerList(std::ostream& os, std::vector<std::list<TreeNode*>>& layerLists);
private:
	TreeNode* m_root;
	void _freeTree(TreeNode*);
	bool _balanceHelper(TreeNode* node);
	void _layerHelper(std::vector<std::list<TreeNode*>>& layerList, TreeNode* node, int layer);
};
